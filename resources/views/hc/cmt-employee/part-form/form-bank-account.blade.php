<section class="row">
    ...
    <div class="col-lg-12 mb-3">
        ...
        <select class="drop-data form-select form-select-solid" data-control="bank_name" name="bank_name" id="bank_name" @unlessrole("administrator") disabled @endunlessrole>
            @if ($user->userBank->name ?? "" == "")
                <option value="" selected hidden disabled>Select bank name</option>
            @endif
            @foreach ($allOptions->bankName as $option)
                <option value="{{$option}}" @if (($user->userBank->name ?? "") == $option) selected @endif>{{$option}}</option>
            @endforeach
        </select>
    ...
    <div class="col-lg-6 mb-3">
        ...
        <input type="number" value="{{$user->userBank->number ?? ""}}" class="form-control form-control-solid" placeholder="Nomor rekening" name="account_number" id="account_number" @unlessrole("administrator") disabled @endunlessrole>
        <div class="fv-plugins-message-container invalid-feedback"></div>
    </div>
    <div class="col-lg-6 mb-3">
        ...
        <input type="text" value="{{$user->userBank->holder_name ?? ""}}" class="form-control form-control-solid" placeholder="Pemilik rekening" name="account_holder_name" id="account_holder_name" @unlessrole("administrator") disabled @endunlessrole>
        <div class="fv-plugins-message-container invalid-feedback"></div>
    </div>
</section>
