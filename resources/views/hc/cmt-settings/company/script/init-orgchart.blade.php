    ...
    function orgInit () {
            ...
            OrgChart.prototype = {
                ...
                init: function(opts) {
                    ...
                    var $chart = this.$chart = $('<div>', {
                        ...
                        'class': 'orgchart' + (this.options.chartClass !== '' ? ' ' + this
                            .options.chartClass : '') + (this.options.direction !==
                    ...
                    var $root = $chart.append($(
                        '<ul class="nodes"><li class="hierarchy"></li></ul>')).find(
                        '.hierarchy');
                    if ($.type(data) === 'object') {
                        if (data instanceof $) { // ul datasource
                            this.buildHierarchy($root, this.buildJsonDS(data.children()), 0, this
                                .options);
                        } else { // local json datasource
                            this.buildHierarchy($root, this.options.ajaxURL ? data : this.attachRel(
                                data, '00'));
                        }
                    } else {
                        $chart.append(
                        ...
                        $.ajax({
                            ...
                            .done(function(data, textStatus, jqXHR) {
                                that.buildHierarchy($root, that.options.ajaxURL ? data : that
                                    .attachRel(data, '00'), 0, that.options);
                            })
                ...
                handleCompactNodes: function() {
                    ...
                    this.$chart.find('.node.compact')
                        .each((index, node) => {
                            $(node).addClass($(node).parents('.compact').length % 2 === 0 ?
                                'even' : 'odd');
                ...
                triggerInitEvent: function() {
                        ...
                        initTime:
                            for (var i = 0; i < mutations.length; i++) {
                                for (var j = 0; j < mutations[i].addedNodes.length; j++) {
                                    if (mutations[i].addedNodes[j].classList.contains(
                                            'orgchart')) {
                                        that.handleCompactNodes();
                                        if (that.options.initCompleted && typeof that
                    ...
                    mo.observe(this.$chartContainer[0], {
                        childList: true
                    });
                },
                ...
                panStartHandler: function(e) {
                    ...
                    if ($(e.target).closest('.node').length || (e.touches && e.touches.length >
                        1)) {
                ...
                panEndHandler: function(e) {
                    if (e.data.chart.data('panning')) {
                        e.data.chart.data('panning', false).css('cursor', 'default').off(
                            'mousemove');
                    }
                },
                //
                ...
                zoomEndHandler: function(e) {
                    ...
                    if (oc.$chart.data('pinching')) {
                        ...
                        var diff = oc.$chart.data('pinchDistEnd') - oc.$chart.data(
                        'pinchDistStart');
                        if (diff > 0) {
                ...
                getPinchDist: function(e) {
                    return Math.sqrt((e.touches[0].clientX - e.touches[1].clientX) * (e.touches[0]
                            .clientX - e.touches[1].clientX) +
                        (e.touches[0].clientY - e.touches[1].clientY) * (e.touches[0].clientY -
                            e.touches[1].clientY));
                },
                //
                ...
                buildJsonDS: function($li) {
                    ...
                    var subObj = {
                        ...
                        'relationship': ($li.parent().parent().is('li') ? '1' : '0') + ($li
                            .siblings('li').length ? 1 : 0) + ($li.children('ul').length ?
                            1 : 0)
                    };
                ...
                attachRel: function(data, flags) {
                    ...
                    if (this.options?.compact?.constructor === Function && this.options.compact(
                            data)) {
                        data.compact = true;
                    }
                    if (data.children) {
                        data.children.forEach(function(item) {
                            if (data.hybrid || data
                                .vertical) { // identify all the descendant nodes except the root node of hybrid structure
                                item.vertical = true;
                            } else if (data.compact && item
                                .children) { // identify all the compact ancestor nodes
                                item.compact = true;
                            } else if (data.compact && !item
                                .children) { // identify all the compact descendant nodes
                ...
                loopChart: function($chart, includeNodeData) {
                    includeNodeData = (includeNodeData !== null && includeNodeData !== undefined) ?
                ...
                getNodeState: function($node, relation) {
                    ...
                    if (relation === 'parent') {
                        ...
                        if ($target.length) {
                            if ($target.is('.hidden') || (!$target.is('.hidden') && $target.closest(
                                    '.nodes').is('.hidden')) || (!$target.is('.hidden') && $target
                                    .closest('.vertical').is('.hidden'))) {
                                return {
                                    'exist': true,
                                    'visible': false
                                };
                        ...
                        $target = isVerticalNode ? $node.parent().children('ul') : $node.siblings(
                            '.nodes');
                        if ($target.length) {
                            if (!$target.is('.hidden')) {
                                return {
                        ...
                        if ($target.length && (!isVerticalNode || $target.children('li').length >
                            1)) {
                            if (!$target.is('.hidden') && !$target.parent().is('.hidden') && (!
                                    isVerticalNode || !$target.closest('.vertical').is('.hidden')
                                    )) {
                                return {
                                    'exist': true,
                                    'visible': true
                        ...
                        $target = $node;
                        if ($target.length) {
                            if (!(($target.closest('.nodes').length && $target.closest('.nodes').is(
                                        '.hidden')) ||
                                    ($target.closest('.hierarchy').length && $target.closest(
                                        '.hierarchy').is('.hidden')) ||
                                    ($target.closest('.vertical').length && ($target.closest(
                                        '.nodes').is('.hidden') || $target.closest(
                                        '.vertical').is('.hidden')))
                                )) {
                                return {
                                    'exist': true,
                ...
                hideParent: function($node) {
                    ...
                    if (this.getNodeState($parent).visible) {
                        $parent.addClass('sliding slide-down').one('transitionend', {
                            'parent': $parent
                        }, this.hideParentEnd);
                    }
                ...
                showParent: function($node) {
                    ...
                    $parent.addClass('sliding').removeClass('slide-down').one('transitionend', {
                        'node': $node
                ...
                hideChildren: function($node) {
                    ...
                    var $animatedNodes = $lowerLevel.find('.node').filter(this.isVisibleNode.bind(
                        this));
                    ...
                    if ($lowerLevel.is('.vertical') || $lowerLevel.find('.vertical').length) {
                        $animatedNodes.find(this.options.icons.expanded).removeClass(this.options
                            .icons.expanded).addClass(this.options.icons.collapsed);
                    }
                ...
                showChildren: function($node) {
                    ...
                    var $animatedNodes = isVerticalDesc ?
                        $levels.removeClass('hidden').find('.node').filter(this.isVisibleNode.bind(
                            this)) :
                        $levels.removeClass('hidden').children('.hierarchy').find('.node:first')
                        .filter(this.isVisibleNode.bind(this));
                    if (!isVerticalDesc) {
                        $animatedNodes.filter(':not(:only-child)').closest('.hierarchy').addClass(
                            'isChildrenCollapsed');
                        $animatedNodes.closest('.hierarchy').removeClass('isCollapsedDescendant');
                    }
                    // the two following statements are used to enforce browser to repaint
                    this.repaint($animatedNodes.get(0));
                    $animatedNodes.addClass('sliding').removeClass('slide-up').eq(0).one(
                        'transitionend', {
                            'node': $node,
                            'animatedNodes': $animatedNodes
                ...
                hideSiblingsEnd: function(event) {
                    ...
                    var $siblings = direction ? (direction === 'left' ? $nodeContainer.prevAll(
                            ':not(.hidden)') : $nodeContainer.nextAll(':not(.hidden)')) :
                ...
                hideSiblings: function($node, direction) {
                    var that = this;
                    var $nodeContainer = $node.closest('.hierarchy').addClass(
                    'isSiblingsCollapsed');
                    ...
                    if (direction) {
                        if (direction === 'left') {
                            $nodeContainer.addClass('left-sibs')
                                .prevAll('.isSiblingsCollapsed').removeClass(
                                    'isSiblingsCollapsed left-sibs').end()
                                .prevAll().addClass('isCollapsedSibling isChildrenCollapsed')
                                .find('.node').filter(this.isVisibleNode.bind(this)).addClass(
                                    'sliding slide-right');
                        } else {
                            $nodeContainer.addClass('right-sibs')
                                .nextAll('.isSiblingsCollapsed').removeClass(
                                    'isSiblingsCollapsed right-sibs').end()
                                .nextAll().addClass('isCollapsedSibling isChildrenCollapsed')
                                .find('.node').filter(this.isVisibleNode.bind(this)).addClass(
                                    'sliding slide-left');
                        }
                    } else {
                        $nodeContainer.prevAll().find('.node').filter(this.isVisibleNode.bind(this))
                            .addClass('sliding slide-right');
                        $nodeContainer.nextAll().find('.node').filter(this.isVisibleNode.bind(this))
                            .addClass('sliding slide-left');
                        $nodeContainer.siblings().addClass(
                        'isCollapsedSibling isChildrenCollapsed');
                    }
                ...
                showSiblingsEnd: function(event) {
                    ...
                    if (this.isInAction($node)) {
                        this.switchHorizontalArrow($node);
                        $node.children('.topEdge').removeClass(this.options.icons.expandToUp)
                ...
                showSiblings: function($node, direction) {
                    ...
                    if (!this.getNodeState($node, 'parent').visible) {
                        ...
                        $upperLevel.addClass('sliding').removeClass('slide-down').one(
                            'transitionend', this.showRelatedParentEnd);
                    }
                    // lastly, show the sibling nodes with animation
                    var $visibleNodes = $siblings.find('.node').filter(this.isVisibleNode.bind(
                        this));
                ...
                startLoading: function($edge) {
                    ...
                    if (typeof $chart.data('inAjax') !== 'undefined' && $chart.data('inAjax') ===
                        true) {
                        return false;
                    }
                ...
                switchHorizontalArrow: function($node) {
                    ...
                    if (opts.toggleSiblingsResp && (typeof opts.ajaxURL === 'undefined' || $node
                            .closest('.nodes').data('siblingsLoaded'))) {
                        var $prevSib = $node.parent().prev();
                        if ($prevSib.length) {
                            if ($prevSib.is('.hidden')) {
                                $node.children('.leftEdge').addClass(opts.icons.collapseToLeft)
                                    .removeClass(opts.icons.expandToRight);
                            } else {
                                $node.children('.leftEdge').addClass(opts.icons.expandToRight)
                                    .removeClass(opts.icons.collapseToLeft);
                            }
                        }
                        var $nextSib = $node.parent().next();
                        if ($nextSib.length) {
                            if ($nextSib.is('.hidden')) {
                                $node.children('.rightEdge').addClass(opts.icons.expandToRight)
                                    .removeClass(opts.icons.collapseToLeft);
                            } else {
                                $node.children('.rightEdge').addClass(opts.icons.collapseToLeft)
                                    .removeClass(opts.icons.expandToRight);
                            }
                        }
                    } else {
                        ...
                        $node.children('.leftEdge').toggleClass(opts.icons.expandToRight,
                            sibsVisible).toggleClass(opts.icons.collapseToLeft, !sibsVisible);
                        $node.children('.rightEdge').toggleClass(opts.icons.collapseToLeft,
                            sibsVisible).toggleClass(opts.icons.expandToRight, !sibsVisible);
                    }
                },
                //
                    ...
                    if ($node.closest('.nodes.vertical').length) {
                        ...
                        if (event.type === 'mouseenter') {
                            if ($node.children('.toggleBtn').length) {
                                ...
                                $toggleBtn.toggleClass(this.options.icons.collapsed, !flag)
                                    .toggleClass(this.options.icons.expanded, flag);
                            }
                        } else {
                        ...
                        if (event.type === 'mouseenter') {
                            if ($topEdge.length) {
                                ...
                                $topEdge.toggleClass(this.options.icons.expandToUp, !flag)
                                    .toggleClass(this.options.icons.collapseToDown, flag);
                            }
                            if ($bottomEdge.length) {
                                flag = this.getNodeState($node, 'children').visible;
                                $bottomEdge.toggleClass(this.options.icons.collapseToDown, !flag)
                                    .toggleClass(this.options.icons.expandToUp, flag);
                            }
                            if ($leftEdge.length) {
                                this.switchHorizontalArrow($node);
                            }
                        } else {
                            $node.children('.edge').removeClass(
                                `${this.options.icons.expandToUp} ${this.options.icons.collapseToDown} ${this.options.icons.collapseToLeft} ${this.options.icons.expandToRight}`
                                );
                        }
                    }
                },
                        ...
                        .done(function(data) {
                            if (that.$chart.data('inAjax')) {
                                if (rel === 'parent') {
                                    ...
                                    that.addSiblings($edge.parent(), data.siblings ? data
                                        .siblings : data);
                                }
                ...
                topEdgeClickHandler: function(event) {
                    ...
                    if (parentState.exist) {
                        ...
                        if (this.startLoading($topEdge)) {
                            var opts = this.options;
                            var url = $.isFunction(opts.ajaxURL.parent) ? opts.ajaxURL.parent($node
                                .data('nodeData')) : opts.ajaxURL.parent + $node[0].id;
                ...
                bottomEdgeClickHandler: function(event) {
                    ...
                    if (childrenState.exist) {
                        ...
                        if (this.startLoading($bottomEdge)) {
                            var opts = this.options;
                            var url = $.isFunction(opts.ajaxURL.children) ? opts.ajaxURL.children(
                                $node.data('nodeData')) : opts.ajaxURL.children + $node[0].id;
                    ...
                    if (siblingsState.exist) {
                        ...
                        if (this.startLoading($hEdge)) {
                            ...
                            var url = (this.getNodeState($node, 'parent').exist) ?
                                ($.isFunction(opts.ajaxURL.siblings) ? opts.ajaxURL.siblings($node
                                    .data('nodeData')) : opts.ajaxURL.siblings + nodeId) :
                                ($.isFunction(opts.ajaxURL.families) ? opts.ajaxURL.families($node
                                    .data('nodeData')) : opts.ajaxURL.families + nodeId);
                ...
                toggleVNodes: function(event) {
                    ...
                    if ($descendants.eq(0).is('.slide-up')) {
                        ...
                        $children.addClass('sliding').removeClass('slide-up').eq(0).one(
                            'transitionend', {
                                'vNodes': $children
                            }, this.expandVNodesEnd);
                    } else {
                        $descendants.addClass('sliding slide-up').eq(0).one('transitionend', {
                            'vNodes': $descendants
                ...
                createGhostNode: function(event) {
                    ...
                    var scale = Math.abs(window.parseFloat(isHorizontal ? transValues[0].slice(
                        transValues[0].indexOf('(') + 1) : transValues[1]));
                    ghostNode.setAttribute('width', isHorizontal ? $nodeDiv.outerWidth(false) :
                        $nodeDiv.outerHeight(false));
                    ghostNode.setAttribute('height', isHorizontal ? $nodeDiv.outerHeight(false) :
                    ...
                    if (isFirefox) { // hack for old version of Firefox(< 48.0)
                        ...
                        ghostNodeWrapper.src = 'data:image/svg+xml;utf8,' + (new XMLSerializer())
                ...
                filterAllowedDropNodes: function($dragged) {
                    ...
                    var $dragHier = $dragged.closest('.hierarchy').find(
                    '.node'); // this node, and its children
                ...
                dropHandler: async function(event) {
                    ...
                    var digger = new JSONDigger(datasource, this.$chart.data('options').nodeId,
                        'children');
                    const hybridNode = digger.findOneNode({
                        'hybrid': true
                    });
                    if (this.$chart.data('options').verticalLevel > 1 || hybridNode) {
                        ...
                        if (!$dropZone.siblings('.nodes')
                            .length) { // if the drop zone is a leaf node
                            $dropZone.append(
                                    `<i class="edge verticalEdge bottomEdge ${this.options.icons.theme}"></i>`
                                    )
                                .after('<ul class="nodes"></ul>')
                                .siblings('.nodes').append($dragged.find('.horizontalEdge').remove()
                                    .end().closest('.hierarchy'));
                            if ($dropZone.children('.title').length) {
                                $dropZone.children('.title').prepend(
                                    `<i class="${this.options.icons.theme} ${this.$chart.data('options').icons.parentNode} parentNodeSymbol"></i>`
                                    );
                            }
                        } else {
                            var horizontalEdges =
                            ...
                            var $dropSibs = $dragged.closest('.hierarchy').siblings().find(
                                '.node:first');
                        ...
                        if ($dragZone.siblings('.nodes').children('.hierarchy').length ===
                            1) { // if there is only one sibling node left
                            $dragZone.siblings('.nodes').children('.hierarchy').find('.node:first')
                                .find('.horizontalEdge').remove();
                        } else if ($dragZone.siblings('.nodes').children('.hierarchy').length ===
                            0) {
                ...
                touchstartHandler: function(event) {
                    ...
                    this.touchMoved =
                    false; // this is so we can work out later if this was a 'press' or a 'drag' touch
                    event.preventDefault();
                },
                //
                touchmoveHandler: function(event) {
                    ...
                    if (!this.touchMoved) {
                        ...
                        this.filterAllowedDropNodes($(event
                        .currentTarget)); // will also set 'this.$chart.data('dragged')' for us
                        // create an image which can be used to illustrate the drag (our own createGhostNode)
                        this.touchDragImage = this.createDragImage(event, this.$chart.data(
                            'dragged')[0]);
                    }
                    ...
                    var $touching = $(document.elementFromPoint(event.touches[0].clientX, event
                        .touches[0].clientY));
                ...
                touchendHandler: function(event) {
                    ...
                    if (this.touchMoved) {
                        ...
                        fakeMouseClickEvent.initMouseEvent('click', true, true, window, 1,
                            firstTouch.screenX, firstTouch.screenY, firstTouch.clientX,
                            firstTouch.clientY, event.ctrlKey, event.altKey, event.shiftKey,
                            event.metaKey, 0, null);
                ...
                moveDragImage: function(event, image) {
                    ...
                    requestAnimationFrame(function() {
                        ...
                        if (orgChartMaster.touchDragImageOffset) {
                            s.left = Math.round(pt.x - orgChartMaster.touchDragImageOffset
                                .x) + 'px';
                            s.top = Math.round(pt.y - orgChartMaster.touchDragImageOffset
                                .y) + 'px';
                        }
                    });
                },
                ...
                createNode: function(data) {
                    ...
                    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (data[
                            opts.nodeId] ? ' id="' + data[opts.nodeId] + '"' : '') + (data
                            .parentId ? ' data-parent="' + data.parentId + '"' : '') + '>')
                        .addClass('node ' + (data.className || '') + (level > opts.visibleLevel ?
                            ' slide-up' : ''));
                    ...
                    if ((opts.verticalLevel && level >= opts.verticalLevel) || data.vertical) {
                        if (Number(flags.substr(2, 1))) {
                            $nodeDiv.append(`<i class="toggleBtn ${opts.icons.theme}"></i>`)
                                .children('.title').prepend(
                                    `<i class="${opts.icons.theme} ${opts.icons.parentNode} parentNodeSymbol"></i>`
                                    );
                        }
                    } else if (data.hybrid) {
                        if (Number(flags.substr(2, 1))) {
                            $nodeDiv.append(
                                    `<i class="edge verticalEdge bottomEdge ${opts.icons.theme}"></i>`
                                    )
                                .children('.title').prepend(
                                    `<i class="${opts.icons.theme} ${opts.icons.parentNode} parentNodeSymbol"></i>`
                                    );
                        ...
                        $nodeDiv.css('grid-template-columns',
                            `repeat(${Math.floor(Math.sqrt(data.children.length + 1))}, auto)`);
                        if (Number(flags.substr(2, 1))) {
                            $nodeDiv.append(`
        ...
        `)
                        ...
                        if (Number(flags.substr(0, 1))) {
                            $nodeDiv.append(
                                `<i class="edge verticalEdge topEdge ${opts.icons.theme}"></i>`);
                        }
                        if (Number(flags.substr(1, 1))) {
                            $nodeDiv.append(
                                `<i class="edge horizontalEdge rightEdge ${opts.icons.theme}"></i><i class="edge horizontalEdge leftEdge ${opts.icons.theme}"></i>`
                                );
                        }
                        if (Number(flags.substr(2, 1))) {
                            $nodeDiv.append(
                    ...
                    $nodeDiv.on('click', '.leftEdge, .rightEdge', this.hEdgeClickHandler.bind(
                    this));
                    $nodeDiv.on('click', '.toggleBtn', this.toggleVNodes.bind(this));
                    $nodeDiv.on('click', '> .backToCompactSymbol', this.backToCompactHandler.bind(
                        this));
                    $nodeDiv.on('click', '> .backToLooseSymbol', this.backToLooseHandler.bind(
                    this));
                ...
                buildHierarchy: function($appendTo, data) {
                    ...
                    if (data.children && data.children.length) {
                        var isHidden = level + 1 > opts.visibleLevel || (data.collapsed !==
                            undefined && data.collapsed);
                        var $nodesLayer;
                        if ((opts.verticalLevel && (level + 1) >= opts.verticalLevel) || data
                            .hybrid) {
                            $nodesLayer = $('<ul class="nodes">');
                            if (isHidden && (opts.verticalLevel && (level + 1) >= opts
                                    .verticalLevel)) {
                ...
                buildParentNode: function($currentRoot, data) {
                    ...
                    this.$chart.prepend($newRootWrapper)
                        .find('.hierarchy:first').append($currentRoot.closest('ul').addClass(
                            'nodes'));
                },
                // exposed method
                ...
                buildSiblingNode: function($nodeChart, data) {
                    ...
                    var existingSibligCount = $nodeChart.parent().is('.nodes') ? $nodeChart
                        .siblings().length + 1 : 1;
                    ...
                    if ($nodeChart.closest('.nodes').parent().is('.hierarchy')) {
                        ...
                        var $siblings = $nodeChart.parent().closest('.hierarchy').children(
                            '.nodes:last').children('.hierarchy');
                        ...
                        this.buildHierarchy($nodeChart.parent().prepend($('<li class="hierarchy">'))
                            .children('.hierarchy:first'), data);
                        $nodeChart.prevAll('.hierarchy').children('.nodes').children().eq(
                            insertPostion).after($nodeChart);
                    }
                },
                //
                ...
                exportPNG: function(canvas, exportFilename) {
                    ...
                    var isEdge = navigator.appName === 'Microsoft Internet Explorer' || (navigator
                    ...
                    if ((!isWebkit && !isFf) || isEdge) {
                        ...
                        var selector = '.download-btn' + (that.options.chartClass !== '' ? '.' +
                            that.options.chartClass : '');

                        if (!$chartContainer.find(selector).length) {
                            $chartContainer.append('<a class="download-btn' + (that.options
                                    .chartClass !== '' ? ' ' + that.options.chartClass : '') +
                                '"' +
                ...
                export: function(exportFilename, exportFileextension) {
                    var that = this;
                    exportFilename = (typeof exportFilename !== 'undefined') ? exportFilename : this
                        .options.exportFilename;
                    exportFileextension = (typeof exportFileextension !== 'undefined') ?
                        exportFileextension : this.options.exportFileextension;
                    ...
                    var sourceChart = $chartContainer.addClass('canvasContainer').find(
                        '.orgchart:not(".hidden")').get(0);
                    var flag = that.options.direction === 'l2r' || that.options.direction === 'r2l';
                    html2canvas(sourceChart, {
                            'width': flag ? sourceChart.clientHeight : sourceChart.clientWidth,
                            'height': flag ? sourceChart.clientWidth : sourceChart.clientHeight,
                            'onclone': function(cloneDoc) {
                                $(cloneDoc).find('.canvasContainer').css('overflow',
                                        'visible')
