<div class="row p-6 m-1 rounded border border-2 border-secondary">
    @can('HC:update-profile')
        <form id="kt_employment_content_form" class="form fv-plugins-bootstrap5 fv-plugins-framework"
            ...
            <div class="col-lg-12 row mb-3 ps-6" id="scheduleShift">
                ...
                @foreach ($dataWorkingScheduleShifts as $workingScheduleShift)
                    <div class="form-check mb-3">
                        <label class="@if ($workingScheduleShift->id == $user->userCurrentShift->working_schedule_shift_id) text-success @endif" for="flexRadioDefault1">
                            {{ $workingScheduleShift->workingShift->name }},
                            {{ $workingScheduleShift->workingShift->working_start }} -
                            {{ $workingScheduleShift->workingShift->working_end }}
                        </label>
                    </div>
                @endforeach
            </div>
        ...
        @can('HC:update-profile')
            <div class="col-lg-12 mt-9 text-end">
                <button type="submit" id="kt_employment_content_submit"
