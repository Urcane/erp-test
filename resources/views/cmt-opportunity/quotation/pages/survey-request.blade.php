    ...
    <div class="row">
        <div class="col-lg-12">
                    ...
                    <ul class="nav nav-tabs flex-nowrap text-nowrap">
                        ...
                        <li class="nav-item">
                            <a class="nav-link fw-semibold btn btn-active-light btn-color-muted btn-active-color-warning rounded-bottom-0"
                                data-bs-toggle="tab" id="tab_on_progress" href="#tab_on_progress_content">On Progress</a>
                        </li>
                        <li class="nav-item">
            ...
            <div class="tab-content" id="myTabContent">
                ...
                <div class="tab-pane fade" id="tab_on_progress" role="tabpanel">
                    <div class="row">
                        <div class="col-lg-12">
                            <table class="table align-top table-striped border table-rounded gy-5"
                                id="kt_table_survey_request">
                                <thead class="">
                ...
                <div class="tab-pane fade" id="tab_done_quotation" role="tabpanel">
                    <div class="row">
                        <div class="col-lg-12">
                            <table class="table align-top table-striped border table-rounded gy-5"
            ...
            generateDatatable({
                ...
                functionCallback: () => {
                    $('body').on('click', '.btn_edit_request_survey', function() {
                        ...
                        $.get(`{{ url('') }}/cmt-survey/survey-request/detail/${id}`,
                            function(data) {
                                form_edit.find('input[name="survey_request_id"]').val(
                                    surveyRequestId);
                                form_edit.find('input[name="no_survey"]').val(data
                                    .no_survey);
                                form_edit.find('select[name="type_of_survey_id"]').val(data
                                    .type_of_survey_id).trigger('change');
                                form_edit.find('select[name="service_type_id"]').val(data
                                    .service_type_id).trigger('change');
                                form_edit.find('input[name="survey_date"]').val(
                                    getFormattedDate(new Date(data.survey_datetime))[0]);
                                form_edit.find('input[name="survey_time"]').val(
                                    getFormattedDate(new Date(data.survey_datetime))[1]);
                                form_edit.find('input[name="lat"]').val(data.lat);
                                form_edit.find('input[name="lang"]').val(data.lang);
                                form_edit.find('input[name="closest_bts"]').val(data
                                    .closest_bts);
                                form_edit.find('textarea[name="notes"]').val(data.notes);

                                prospectIds.push(data.customer_prospect_id);
                                $.each(prospectIds.filter(onlyUnique), function(index,
                                    rowId) {
                                    form_edit.find('#containerSelectedProspects')
                                        .append(
                                            $('<input>')
                                            .attr('type', 'hidden')
                                            .attr('name', 'prospect_id[]')
            ...
            $('body').on('click', '.btn_create_soft_survey', function() {
                ...
                form_edit.on('click', '.btn_add_more_soft_survey_item', function() {
                    ...
                    $(`.clear-soft-survey-item-${random_string}`).click(function() {
                        ...
                        $('#countable_soft_survey_items').html($('.soft-survey-item')
                            .length + 1);
                    })
